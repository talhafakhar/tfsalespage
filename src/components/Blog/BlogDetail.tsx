...
const BlogDetail: React.FC<BlogCardProps> = ({ blog }) => {
    ...
    return (
        <div className="min-h-screen">
            ...
            <div className="fixed top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <motion.div
                    className="absolute top-20 right-20 w-[600px] h-[300px] bg-primary/20 rounded-full blur-3xl"
                    animate={{
                        x: [0, 50, 0],
                        y: [0, -30, 0],
                        scale: [1, 1.1, 1]
                ...
                />
                <motion.div
                    className="absolute bottom-20 left-20 w-[500px] h-[250px] bg-primary/20 rounded-full blur-3xl"
                    animate={{
                        x: [0, -40, 0],
                        y: [0, 40, 0],
                        scale: [1, 0.9, 1]
            ...
            >
                <div className="max-w-5xl mx-auto px-6 lg:px-8">
                    <motion.header className="mb-12" variants={fadeInUp}>
                        ...
                        >
                            {blog.tags?.split(',').map((tag, index) => (
                                <motion.span
                                    key={index}
                                    className="px-4 py-2 bg-white/80 backdrop-blur-sm border border-secondary/10 text-sm font-medium text-gray-700 rounded-full shadow-sm hover:shadow-md transition-all duration-300 hover:scale-105"
                        ...
                        <motion.h1
                            className="text-4xl md:text-6xl lg:text-7xl font-bold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 bg-clip-text text-transparent leading-tight mb-8"
                            variants={fadeInUp}
                        >
                            {blog.title}
                        </motion.h1>
                    ...
                    {/* Content */}
                    <motion.article
                        className="prose prose-lg lg:prose-xl max-w-none mb-16"
                        variants={fadeInUp}
                        style={{
                    ...
                    >
                        <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-8 lg:p-12 shadow-lg border border-gray-200/50">
                            {blog.content.map((block, index: number) => {
                                    ...
                                    >
                                        <Tag className="mb-6 text-lg leading-relaxed">
                                            {block.children.map((child: any, i: number) => {
                                                ...
                                                else if (child.italic) InlineTag = "em";

                                                return (
                                                    <InlineTag key={i} className={child.bold ? "font-semibold" : ""}>
                                                        {child.text}
                                                    </InlineTag>
                                                );
                                            })}
                    ...
                    <motion.footer
                        className="py-8 border-t border-gray-200/50"
                        variants={fadeInUp}
                    >
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                            <Link
                                href="/blogs"
                                className="group inline-flex items-center px-8 py-4 bg-primary backdrop-blur-sm font-semibold rounded-full border border-gray-200/50 hover:border-gray-300 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                            >
                                <svg
